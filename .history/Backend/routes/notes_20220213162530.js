...
router.put('/updatenote/:id', fetchuser, async (req, res) => {
  ...
  try {
      ...
      if (noteTOBEUpdated.user.toString() !== req.body.id) {
        return res.status(401).send('Not        let note = await Note.findById(req.params.id);
        if(!note) { return res.status(404).send("Not Found") }Allowed')
      }
