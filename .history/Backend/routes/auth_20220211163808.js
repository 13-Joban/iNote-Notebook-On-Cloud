...
async (req, res) => {
 ...
 if (!user) {
   return res.status(404).json({ error: "Please login using correct ceredenti" })
 }

 const passworrdCompare = bcrypt.compare(password, user.password);
 if( !passworrdCompare){
  return res.status(404).json({ error: "Please login using correct Details" })
 }
