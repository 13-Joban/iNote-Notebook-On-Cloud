...
// salt is appended by the backend to protect from getting hacked by the hacker which have already rainbow box containing all the hashes 
 ...
 if (user) {
   return res.status(400).json({ error: "Sorry a user with this Details already exists" })
 }
...
// console.log(authtoken);
  ...
  User.create({
    ...
    }).then( (user) =>  res.json(generateauthtoken(user)))

    }
    .catch( err => res.json({err: "please enter unique user details"}));
