...
// let  user = User(req.body);
 function generateauthtoken(user) {  let data  = {
  user: {
      id: user.id
  }
}
console.log(user.id);
  let  authtoken = jwt.sign( user.id,  JWT_SECRET);
...
// console.log(authtoken);
  ...
  User.create({
    ...
    }).then( (user) =>  res.json(generateauthtoken(user)))  //  we will not send the user in response but a  auth token
