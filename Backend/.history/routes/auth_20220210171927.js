...
// console.log(authtoken);
  ...
  User.create({
    ...
    }).then( (user) =>  res.json(generateauthtoken(user)))  //  we will not send the user in response but a  auth token.catch( err => res.json({err: "please enter unique user details"}));
    // res.send(req.body)
